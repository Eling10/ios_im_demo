...
static ELAudioPlayerHelper *playerHelper = nil;
@interface ELAudioPlayerHelper()<AVAudioPlayerDelegate>
...
@implementation ELAudioPlayerHelper
...
+ (instancetype)sharedHelper
{
    ...
    dispatch_once(&onceToken, ^{
        playerHelper = [[ELAudioPlayerHelper alloc] init];
    });
    return playerHelper;
}
...
- (NSString *)_convertAudioFile:(NSString *)aPath
{
    ...
    do {
        ...
        if ([ELAudioPlayerHelper isMP3File:retPath]) {
            retPath = aPath;
            break;
        }
        
        [ELAudioPlayerHelper amrToWav:aPath wavSavePath:retPath];
        if (![fileManager fileExistsAtPath:retPath]) {
            retPath = nil;
        }
